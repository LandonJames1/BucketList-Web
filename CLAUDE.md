...
| Token | Face | Used for |
| --- | --- | --- |
| `--serif` | Cormorant Garamond | Display only: screen titles, collection and activity names, stat numerals, sheet titles, and long-form completion notes (`.ad-note.prose`). |
...
| File | Domain |
| --- | --- |
| `auth.js` | `showAuth`/`showApp` (swap `#authPage` against `#appWrap`; `showApp` boots into the Lists tab, loads the profile, and triggers the iOS install hint), `toggleAuthMode` (tracked by the `authIsSignUp` flag, not by reading the heading text), `setAuthError`, `handleAuth`, `handleSignOut`. Sign-up also inserts the `Users` profile row. |
...
| `main.js` | Boot: `paintStaticIcons()` fills the empty icon placeholders left in `index.html` from the sprite map, then the Supabase session is restored and `showApp()`/`showAuth()` runs. **Loads last.** |
